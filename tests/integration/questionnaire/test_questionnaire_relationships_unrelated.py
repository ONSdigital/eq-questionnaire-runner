...
class TestQuestionnaireRelationshipsUnrelated(QuestionnaireTestCase):
    def test_unrelated_relationship_is_accessible_when_list_not_empty(self):
        self.launchSurvey(
            "test_relationships_unrelated_relationships", roles=["dumper"]
        )
        ...
        self.get(
            f"/questionnaire/relationships/{first_list_item}/related-to-anyone-else"
        )
    ...
    def test_unrelated_relationship_is_not_accessible_invalid_list_item(self):
        self.launchSurvey("test_relationships_unrelated_relationships")
        self.get(f"/questionnaire/relationships/invalid-id/related-to-anyone-else")
