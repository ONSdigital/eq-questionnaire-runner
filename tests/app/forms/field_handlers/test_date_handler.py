...
def test_date_field_created_with_guidance(rule_evaluator):
    date_json = {
    ...
    handler = DateHandler(date_json, rule_evaluator, error_messages)
...
def test_generate_date_form_validates_single_date_period_with_bespoke_message(
    ...
    rule_evaluator.value_source_resolver.schema = load_schema_from_name(
        "test_date_validation_single"
    )
    answer = {
        "id": "date-range-from",
        "mandatory": True,
    ...
    handler = DateHandler(answer, rule_evaluator, error_messages)
    form = date_field.get_form_class(handler.validators)
...
def test_get_referenced_offset_value_for_value(app, rule_evaluator):
...
def test_get_referenced_offset_value_for_now_value(app, rule_evaluator):
...
def test_get_referenced_offset_value_for_meta(app, rule_evaluator):
    rule_evaluator.value_source_resolver.data_stores.metadata = {"date": "2018-02-20"}
...
def test_get_referenced_offset_value_for_answer_id(app, rule_evaluator):
...
def test_get_referenced_offset_value_with_list_item_id(app, rule_evaluator):
    list_item_id = "abcde"
    ...
    answer_store.add_or_update(test_answer_id)
    rule_evaluator.value_source_resolver.data_stores.answer_store = answer_store
    rule_evaluator.value_source_resolver.location = location
    rule_evaluator.value_source_resolver.list_item_id = list_item_id
    answer = {
        "maximum": {
...
def test_get_referenced_offset_value_with_no_offset(app, rule_evaluator):
    answer = {"minimum": {"value": "2017-06-11"}}
...
def test_minimum_and_maximum_offset_dates(app, rule_evaluator):
    rule_evaluator.value_source_resolver.data_stores.metadata = {"date": "2018-02-20"}
    answer_store = AnswerStore()
...
def test_greater_minimum_date_than_maximum_date(app, rule_evaluator):
    answer = {
        "id": "date_answer",
        "type": "Date",
