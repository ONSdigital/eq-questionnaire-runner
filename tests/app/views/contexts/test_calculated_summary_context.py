...
def test_build_view_context_for_currency_calculated_summary(
    ...
    assert (
        len(context_summary["sections"]["default-section"]["groups"][0]["blocks"])
        == total_blocks
    )
    assert (
    ...
    answer_change_link = context_summary["sections"]["default-section"]["groups"][0][
        "blocks"
    ][0]["question"]["answers"][0]["link"]
