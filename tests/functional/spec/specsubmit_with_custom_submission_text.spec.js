...
describe("Submit with custom submission text", () => {
  ...
  it("Given a questionnaire with custom submission content has been started, when it is completed to the submit page, then the correct submission content should be displayed", async ()=> {
    ...
    await $(await BreakfastPage.answer()).setValue("Eggs");
    await $(await BreakfastPage.submit()).click();
    await expect(await $(await SubmitPage.heading()).getText()).to.contain("Submit your questionnaire");
    await expect(await $(await SubmitPage.warning()).getText()).to.contain("You cannot view your answers after submission");
    await expect(await $(await SubmitPage.guidance()).getText()).to.contain("Thank you for your answers, submit this to complete it");
  });
});
