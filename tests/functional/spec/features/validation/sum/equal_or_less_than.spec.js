...
describe("Feature: Sum of grouped answers validation (equal or less than) against total", () => {
  ...
  describe("Given I start a grouped answer validation survey and enter 12 into the total", () => {
    it("When I continue and enter 4 in each breakdown field, Then I should see a validation error", async () => {
      ...
      await click(TotalAnswerPage.submit());
      await $(BreakdownAnswerPage.breakdown1()).setValue("4");
      await $(BreakdownAnswerPage.breakdown2()).setValue("4");
      await $(BreakdownAnswerPage.breakdown3()).setValue("4");
      await $(BreakdownAnswerPage.breakdown4()).setValue("4");
      await click(BreakdownAnswerPage.submit());
      await expect(await $(BreakdownAnswerPage.errorNumber(1)).getText()).to.contain("Enter answers that add up to or are less than 12");
    });
  });
  ...
  describe("Given I start a grouped answer validation survey and enter 5 into the total", () => {
    it("When I continue and enter 3 in each breakdown field, Then I should see a validation error", async () => {
      ...
      await $(BreakdownAnswerPage.breakdown4()).setValue("3");
      await click(BreakdownAnswerPage.submit());
      await expect(await $(BreakdownAnswerPage.errorNumber(1)).getText()).to.contain("Enter answers that add up to or are less than 5");
    });
  });
});
