...
describe("Feature: Sum of grouped answers validation (equal or less than) against total", () => {
  beforeEach(async ()=> {
    await browser.openQuestionnaire("test_validation_sum_against_total_equal_or_less_than.json");
  });
  ...
  describe("Given I start a grouped answer validation survey and enter 12 into the total", () => {
    it("When I continue and enter 4 in each breakdown field, Then I should see a validation error", async ()=> {
      await $(await TotalAnswerPage.total()).setValue("12");
      await $(await TotalAnswerPage.submit()).click();
      await $(await BreakdownAnswerPage.breakdown1()).setValue("4");
      await $(await BreakdownAnswerPage.breakdown2()).setValue("4");
      await $(await BreakdownAnswerPage.breakdown3()).setValue("4");
      await $(await BreakdownAnswerPage.breakdown4()).setValue("4");
      await $(await BreakdownAnswerPage.submit()).click();
      await expect(await $(await BreakdownAnswerPage.errorNumber(1)).getText()).to.contain("Enter answers that add up to or are less than 12");
    });
  });
  ...
  describe("Given I start a grouped answer validation survey and enter 5 into the total", () => {
    it("When I continue and enter 3 in each breakdown field, Then I should see a validation error", async ()=> {
      ...
      await $(await BreakdownAnswerPage.breakdown4()).setValue("3");
      await $(await BreakdownAnswerPage.submit()).click();
      await expect(await $(await BreakdownAnswerPage.errorNumber(1)).getText()).to.contain("Enter answers that add up to or are less than 5");
    });
  });
});
