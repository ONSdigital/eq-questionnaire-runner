  ...
  describe("Period Validation", () => {
    describe("Given I enter dates", () => {
      it("When I enter dates that are too early and too late, Then I should see two validation errors", async () => {
        ...
        await expect(await $(DateRangePage.errorNumber(1)).getText()).toContain("Enter a date after 2015");
        await expect(await $(DateRangePage.errorNumber(2)).getText()).toContain("Enter a date before 2021");
      });
      ...
      it("When I enter a range too large, Then I should see a range validation error", async () => {
        ...
        await click(DateRangePage.submit());
        await expect(await $(DateRangePage.errorNumber(1)).getText()).toContain("Enter a reporting period less than or equal to 3 years");
      });
      ...
      it("When I enter a range too small, Then I should see a range validation error", async () => {
        ...
        await click(DateRangePage.submit());
        await expect(await $(DateRangePage.errorNumber(1)).getText()).toContain("Enter a reporting period greater than or equal to 2 years");
      });
      ...
      it("When I enter valid dates, Then I should see the summary page", async () => {
        ...
        await click(DateRangePage.submit());
        await expect(await $(SubmitPage.dateRangeFrom()).getText()).toContain("2016 to 2018");

        // Max range
        await $(SubmitPage.dateRangeFromEdit()).click();
        await $(DateRangePage.dateRangeToYear()).setValue(2019);
        await click(DateRangePage.submit());
        await expect(await $(SubmitPage.dateRangeFrom()).getText()).toContain("2016 to 2019");
      });
    });
  });
});
