  ...
  describe("Period Validation", () => {
    describe("Given I enter a date period greater than the max period limit", () => {
      it("When I continue, Then I should see a period validation error", async ()=> {
        ...
        await expect(await $(await DateRangePage.errorNumber(1)).getText()).to.contain("Enter a reporting period less than or equal to 1 month, 20 days");
      });
    });
    ...
    describe("Given I enter a date period less than the min period limit", () => {
      it("When I continue, Then I should see a period validation error", async ()=> {
        ...
        await $(await DateRangePage.dateRangeToyear()).setValue(2018);
        await $(await DateRangePage.submit()).click();
        await expect(await $(await DateRangePage.errorNumber(1)).getText()).to.contain("Enter a reporting period greater than or equal to 23 days");
      });
    });
  ...
  describe("Date Range Validation", () => {
    describe('Given I enter a "to date" which is earlier than the "from date"', () => {
      it("When I continue, Then I should see a validation error", async ()=> {
        await $(await DateRangePage.dateRangeFromday()).setValue(1);
        await $(await DateRangePage.dateRangeFrommonth()).setValue(2);
        await $(await DateRangePage.dateRangeFromyear()).setValue(2018);
