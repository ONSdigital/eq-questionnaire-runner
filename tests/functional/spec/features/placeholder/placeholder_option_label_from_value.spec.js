...
describe("Option label value check", () => {
  ...
  it("Given radio options are provided, when I select first answer (piped from metadata) and go to the next page, then the question title contains the label text of the answer I selected", async ()=> {
    await expect(await $(await MandatoryRadioPage.answerBusinessNamePipedLabel()).getText()).to.contain("Apple (piped)");
    await $(await MandatoryRadioPage.answerBusinessNamePiped()).click();
    await $(await MandatoryRadioPage.submit()).click();
    await expect(await $(await ConfirmationQuestionRadioBlockPage.questionText()).getText()).to.contain("Apple (piped)");
  });
  ...
  it("Given radio options are provided, when I select an answer (static) and go to the next page, then the question title contains the label text of the answer I selected", async ()=> {
    await $(await MandatoryRadioPage.googleLtd()).click();
    await $(await MandatoryRadioPage.submit()).click();
    await expect(await $(await ConfirmationQuestionRadioBlockPage.questionText()).getText()).to.contain("Google LTD");
  });
});
