...
describe("Date checks", () => {
  ...
  it("Given a reference date is provided, when I get to the next page, then the placeholder contains a formatted date range based on the reference date", async ()=> {
    await $(await DateQuestionPage.day()).setValue(8);
    await $(await DateQuestionPage.month()).setValue(9);
    await $(await DateQuestionPage.year()).setValue(2021);

    await $(await DateQuestionPage.submit()).click();

    await expect(await $(await DaysQuestionBlockPage.questionText()).getText()).to.contain("Monday 30 August to Monday 13 September 2021");
    await $(await DaysQuestionBlockPage.submit()).click();
  });
  ...
  it("Given a reference date is provided, when I get to the next page, then the placeholder contains a formatted date range", async ()=> {
    await $(await DateQuestionPage.day()).setValue(15);
    await $(await DateQuestionPage.month()).setValue(9);
    await $(await DateQuestionPage.year()).setValue(2021);
    ...
    await $(await Block0Page.ref0day()).setValue(1);
    await $(await Block0Page.ref0month()).setValue(5);
    await $(await Block0Page.ref0year()).setValue(2019);

    await $(await Block0Page.ref1day()).setValue(19);
    await $(await Block0Page.ref1month()).setValue(5);
    await $(await Block0Page.ref1year()).setValue(2019);

    await $(await Block0Page.submit()).click();

    await expect(await $(await RangeQuestionBlockPage.questionText()).getText()).to.contain("Wednesday 1 to Sunday 19 May 2019");
  });
});
