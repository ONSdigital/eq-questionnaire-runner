    ...
    describe("Summary page", () => {
      ...
      describe("Given a dynamic answer options questionnaire", () => {
        it("When I submit my questions without answering, then the summary should display `No answer provided` for each question", async () => {
          ...
          await expect(await $(SubmitPage.dynamicRadioAnswer()).getText()).to.equal("No answer provided");
          await expect(await $(SubmitPage.dynamicDropdownAnswer()).getText()).to.equal("No answer provided");
        ...
        it("When I select a dynamically generated answer option for each question, then my selected answers should be displayed on the summary", async () => {
          ...
          await expect(await $(SubmitPage.dynamicRadioAnswer()).getText()).to.equal("Tuesday 29 December 2020");
          await expect(await $(SubmitPage.dynamicDropdownAnswer()).getText()).to.equal("Saturday 2 January 2021");
          await expect(await $(SubmitPage.dynamicMutuallyExclusiveDynamicAnswer()).getText()).to.equal("Monday 28 December 2020\nSunday 3 January 2021");
        });
      });
    });
  });
...
describe(`Feature: Dynamically generated answer options driven by a function with static options`, () => {
  describe("Given a dynamic answer options questionnaire with static options", () => {
    ...
    it("When I select a static answer option for each question, then my selected answer(s) should be displayed on the summary", async () => {
      ...
      await expect(await $(SubmitPage.dynamicRadioAnswer()).getText()).to.equal("I did not work");
      await expect(await $(SubmitPage.dynamicDropdownAnswer()).getText()).to.equal("I did not work");
