...
describe("Feature: Routing rules based on progress value sources", () => {
  ...
  describe("Given I have routing based on the completeness of a block", () => {
    it("When the block being evaluated is incomplete (Q2), Then the dependent question (Q4) should not be displayed", async () => {
      ...
      await expect(await browser.getUrl()).to.contain(FifthQuestionPage.pageName);
      await $(FifthQuestionPage.q1A1()).setValue("2");
      await $(FifthQuestionPage.submit()).click();

      await expect(await browser.getUrl()).to.contain(SeventhQuestionPage.pageName);
    ...
    describe("Given I have routing based on the completeness of a block", () => {
    it("When the blocks being evaluated are complete (Q2 + Q5), Then the dependent questions (Q4 + Q6) should be displayed", async () => {
      ...
      await expect(await browser.getUrl()).to.contain(SubmitPage.pageName);
      await expect(await $("body").getText()).to.have.string("Section 1 Question 2");
      await expect(await $("body").getText()).to.have.string("Section 1 Question 5");
    });
  });
});
