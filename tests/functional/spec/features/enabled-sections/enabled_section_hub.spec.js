...
describe("Feature: Section Enabled With Hub", () => {
  ...
  it("When the user selects `Section 2` and submits, Then only section 2 should be displayed on the hub", async ()=> {
    ...
    await expect(await $(await hubPage.summaryRowState("section-2")).isDisplayed()).to.be.true;
    await expect(await $(await hubPage.summaryRowTitle("section-2")).getText()).to.equal("Section 2");

    await expect(await $(await hubPage.summaryRowState("section-3")).isDisplayed()).to.be.false;
  });
  ...
  it("When the user selects `Section 3` and submits, Then section 2 should not be displayed and section 3 should be displayed", async ()=> {
    ...
    await expect(await $(await hubPage.summaryRowState("section-3")).isDisplayed()).to.be.true;
    await expect(await $(await hubPage.summaryRowTitle("section-3")).getText()).to.equal("Section 3");

    await expect(await $(await hubPage.summaryRowState("section-2")).isDisplayed()).to.be.false;
  });
