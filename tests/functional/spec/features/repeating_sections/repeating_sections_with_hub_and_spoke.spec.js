...
describe("Feature: Repeating Sections with Hub and Spoke", () => {
  describe("Given the user has added some members to the household and is on the Hub", () => {
    ...
    it("Then a section for each household member should be displayed", () => {
      ...
      expect($(HubPage.summaryRowState("section-1")).getText()).to.equal("Completed");
      expect($(HubPage.summaryRowState("section-2")).getText()).to.equal("Not started");
      expect($(HubPage.summaryRowTitle("section-2")).getText()).to.equal("Marcus Twin");
      expect($(HubPage.summaryRowState("section-3")).getText()).to.equal("Not started");
      expect($(HubPage.summaryRowTitle("section-3")).getText()).to.equal("Jean Clemens");
      expect($(HubPage.summaryRowState("section-4")).getText()).to.equal("Not started");
      expect($(HubPage.summaryRowTitle("section-4")).getText()).to.equal("Samuel Clemens");
      expect($(HubPage.summaryRowState("section-5")).getText()).to.equal("Not started");
      expect($(HubPage.summaryRowTitle("section-5")).getText()).to.equal("John Doe");
    ...
    it("When the user partially completes a repeating section, Then that section should be marked as 'Partially completed' on the Hub", () => {
      ...
      expect(browser.getUrl()).to.contain(HubPage.url());
      expect($(HubPage.summaryRowState("section-2")).getText()).to.equal("Partially completed");

      expect($(HubPage.summaryRowTitle("section-2")).getAttribute("class")).to.not.contain("summary__item-title--has-icon");
    });
    ...
    it("When the user completes a repeating section, Then that section should be marked as 'Completed' on the Hub", () => {
      ...
      expect(browser.getUrl()).to.contain(HubPage.url());
      expect($(HubPage.summaryRowState("section-3")).getText()).to.equal("Completed");

      expect($(HubPage.summaryRowTitle("section-3")).getAttribute("class")).to.contain("summary__item-title--has-icon");
    });
    ...
    it("When the user adds 2 visitors to the household then a section for each visitor should be display on the hub", () => {
      ...
      expect($(HubPage.summaryRowState("section-6")).getText()).to.equal("Not started");
      expect($(HubPage.summaryRowTitle("section-6")).getText()).to.equal("Joe Public");
      expect($(HubPage.summaryRowState("section-7")).getText()).to.equal("Not started");
      expect($(HubPage.summaryRowTitle("section-7")).getText()).to.equal("Yvonne Yoe");

      expect($(HubPage.summaryRowState("section-8")).isExisting()).to.be.false;
    });
    ...
    it("When the user answers on their behalf, Then they are shown the non proxy question variant", () => {
      $(HubPage.summaryRowLink("section-5")).click();
    ...
    it("When the user answers on on behalf of someone else, Then they are shown the proxy question variant for the relevant repeating section", () => {
      $(HubPage.summaryRowLink("section-4")).click();
      $(ProxyPage.yes()).click();
      $(ProxyPage.submit()).click();
    ...
    it("When the user adds a new member to the household, Then the Hub should not be in the completed state", () => {
      ...
      expect($(HubPage.summaryRowState("section-8")).getText()).to.equal("Not started");
      expect($(HubPage.summaryRowState("section-8")).isExisting()).to.be.true;
