...
describe("Given I start a survey", () => {
  ...
  it("When I view interstitial page and the interviewer_note is set to true then I should be able to see interviewer note", async ()=> {
    await expect(await $(await InitialInterstitialPage.questionText()).getText()).to.contain("Interviewer note");
  });
  it("When I view question page and the interviewer_note is set to true then I should be able to see interviewer note", async ()=> {
    await $(await InitialInterstitialPage.submit()).click();
    await expect(await $(await FavouriteTeamPage.questionText()).getText()).to.contain("Interviewer note");
  });
  it("When I view question page and the interviewer_note is set to false then I should not be able to see interviewer note", async ()=> {
    await $(await FavouriteTeamPage.favouriteTeam()).setValue("TNS");
    await $(await FavouriteTeamPage.submit()).click();
    await expect(await $(await ConfirmPage.questionText()).getText()).to.not.contain("Interviewer note");
  });
  it("When I view interstitial page and the interviewer_note is not set then I should not be able to see interviewer note", async ()=> {
    await $(await ConfirmPage.yes()).click();
    await $(await ConfirmPage.submit()).click();
    await expect(await $(await FinalInterstitialPage.questionText()).getText()).to.not.contain("Interviewer note");
  });
});
