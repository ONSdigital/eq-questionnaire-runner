import ListCollectorPage from "../generated_pages/relationships_unrelated/list-collector.page.js";
import ListCollectorAddPage from "../generated_pages/relationships_unrelated/list-collector-add.page.js";
import RelationshipsPage from "../generated_pages/relationships_unrelated/relationships.page.js";
import RelatedToAnyoneElsePage from "../generated_pages/relationships_unrelated/related-to-anyone-else.page.js";
import RelationshipsInterstitialPage from "../generated_pages/relationships_unrelated/relationship-interstitial.page.js";

describe("Unrelated Relationships", () => {
  const schema = "test_relationships_unrelated.json";

  describe("Given I am completing the test_relationships_unrelated survey,", () => {
    before("load the survey", async ()=> {
      await browser.openQuestionnaire(schema);
    });

    describe("And I add six people", () => {
      before("add people", async ()=> {
        addPerson("Andrew", "Austin");
        addPerson("Betty", "Burns");
        addPerson("Carla", "Clark");
        addPerson("Daniel", "Davis");
        addPerson("Eve", "Elliot");
        await $(await ListCollectorPage.no()).click();
        await $(await ListCollectorPage.submit()).click();
      });

      it("When I answer 'Unrelated' twice, Then I will be asked if anyone else is related with a list of the remaining people", async ()=> {
        await $(await RelationshipsPage.unrelated()).click();
        await $(await RelationshipsPage.submit()).click();
        await $(await RelationshipsPage.unrelated()).click();
        await $(await RelationshipsPage.submit()).click();
        await expect(await $(await RelatedToAnyoneElsePage.questionText()).getText()).to.contain("Are any of these people related to you?");
        await expect(await $(await RelatedToAnyoneElsePage.listLabel(1)).getText()).to.equal("Daniel Davis");
        await expect(await $(await RelatedToAnyoneElsePage.listLabel(2)).getText()).to.equal("Eve Elliot");
      });

      it("When I click previous, Then I will go back to the previous relationship", async ()=> {
        await $(await RelatedToAnyoneElsePage.previous()).click();
        await expect(await $(await RelationshipsPage.questionText()).getText()).to.contain("Carla Clark is unrelated to Andrew Austin");
      });

      it("When I return to the 'related to anyone else' question and select 'Yes', Then I will be taken to the next relationship for the first person", async ()=> {
        await $(await RelationshipsPage.submit()).click();
        await $(await RelatedToAnyoneElsePage.yes()).click();
        await $(await RelatedToAnyoneElsePage.submit()).click();
        await expect(await $(await RelationshipsPage.questionText()).getText()).to.contain("Thinking about Andrew Austin, Daniel Davis is their");
      });

      it("When I click previous, Then I will go back to the 'related to anyone else' question", async ()=> {
        await $(await RelationshipsPage.previous()).click();
        await expect(await $(await RelatedToAnyoneElsePage.questionText()).getText()).to.contain("Are any of these people related to you?");
        await expect(await $(await RelatedToAnyoneElsePage.yes()).isSelected()).to.be.true;
      });

      it("When I select 'No' to the 'related to anyone else' question, Then I will be taken to the first relationship for the second person", async ()=> {
        await $(await RelatedToAnyoneElsePage.noNoneOfThesePeopleAreRelatedToMe()).click();
        await $(await RelatedToAnyoneElsePage.submit()).click();
        await expect(await $(await RelationshipsPage.questionText()).getText()).to.contain("Thinking about Betty Burns, Carla Clark is their");
      });

      it("When I click previous, Then I will go back to the 'related to anyone else' question for the first person", async ()=> {
        await $(await RelationshipsPage.previous()).click();
        await expect(await $(await RelatedToAnyoneElsePage.questionText()).getText()).to.contain("Are any of these people related to you?");
        await expect(await $(await RelatedToAnyoneElsePage.listLabel(1)).getText()).to.equal("Daniel Davis");
        await expect(await $(await RelatedToAnyoneElsePage.listLabel(2)).getText()).to.equal("Eve Elliot");
        await expect(await $(await RelatedToAnyoneElsePage.noNoneOfThesePeopleAreRelatedToMe()).isSelected()).to.be.true;
      });

      it("When I click complete the remaining relationships, Then I will go to the relationships section complete page", async ()=> {
        await $(await RelatedToAnyoneElsePage.submit()).click();
        await $(await RelationshipsPage.unrelated()).click();
        await $(await RelationshipsPage.submit()).click();
        await $(await RelationshipsPage.unrelated()).click();
        await $(await RelationshipsPage.submit()).click();
        await $(await RelationshipsPage.unrelated()).click();
        await $(await RelationshipsPage.submit()).click();
        await $(await RelationshipsPage.unrelated()).click();
        await $(await RelationshipsPage.submit()).click();
        await $(await RelationshipsPage.unrelated()).click();
        await $(await RelationshipsPage.submit()).click();
        await $(await RelationshipsPage.unrelated()).click();
        await $(await RelationshipsPage.submit()).click();
        await expect(browser.getUrl()).to.contain(RelationshipsInterstitialPage.pageName);
      });
    });

    function addPerson(firstName, lastName) {
      await $(await ListCollectorPage.yes()).click();
      await $(await ListCollectorPage.submit()).click();
      await $(await ListCollectorAddPage.firstName()).setValue(firstName);
      await $(await ListCollectorAddPage.lastName()).setValue(lastName);
      await $(await ListCollectorAddPage.submit()).click();
    }
  });
});
