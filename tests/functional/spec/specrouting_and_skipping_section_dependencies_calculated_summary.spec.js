...
describe("Routing and skipping section dependencies based on calculated summaries", () => {
  describe("Given the section dependencies based on a calculated summary questionnaire", () => {
    beforeEach("Load the survey", async ()=> {
      await browser.openQuestionnaire("test_new_routing_and_skipping_section_dependencies_calculated_summary.json");
    });

    it("When the calculated summary total has not been set, Then the dependent section should not be enabled", async ()=> {
      await expect(await $(await HubPage.summaryRowLink("calculated-summary-section")).isExisting()).to.be.true;
      await expect(await $(await HubPage.summaryRowLink("dependent-question-section")).isExisting()).to.be.true;
      await expect(await $(await HubPage.summaryRowLink("dependent-enabled-section")).isExisting()).to.be.false;
    });

    it("When the calculated summary total is equal to £100, Then the dependent section should be enabled", async ()=> {
      await $(await HubPage.summaryRowLink("calculated-summary-section")).click();
      await $(await FirstQuestionBlockPage.milk()).setValue(25);
      await $(await FirstQuestionBlockPage.eggs()).setValue(25);
      await $(await FirstQuestionBlockPage.bread()).setValue(25);
      await $(await FirstQuestionBlockPage.cheese()).setValue(25);
      await $(await FirstQuestionBlockPage.submit()).click();
      await $(await CurrencyTotalPlaybackPage.submit()).click();
      await $(await CalculatedSummarySectionSummaryPage.submit()).click();

      await expect(await $(await HubPage.summaryRowLink("calculated-summary-section")).isExisting()).to.be.true;
      await expect(await $(await HubPage.summaryRowLink("dependent-question-section")).isExisting()).to.be.true;
      await expect(await $(await HubPage.summaryRowLink("dependent-enabled-section")).isExisting()).to.be.true;
    });
    ...
    it("When a question in another section has a skip condition dependency on a calculated summary total, and the skip condition is not met (total less than £10), then the dependent question should be displayed", async ()=> {
      await $(await HubPage.summaryRowLink("calculated-summary-section")).click();
      await $(await FirstQuestionBlockPage.milk()).setValue(1);
      await $(await FirstQuestionBlockPage.eggs()).setValue(1);
      await $(await FirstQuestionBlockPage.bread()).setValue(1);
      await $(await FirstQuestionBlockPage.cheese()).setValue(1);
      await $(await FirstQuestionBlockPage.submit()).click();
      await $(await CurrencyTotalPlaybackPage.submit()).click();
      await $(await CalculatedSummarySectionSummaryPage.submit()).click();

      await $(await HubPage.summaryRowLink("dependent-question-section")).click();
    ...
    it("When a question in another section has a skip condition dependency on a calculated summary total, and the skip condition is met (total greater than £10), then the dependent question should not be displayed", async ()=> {
      await $(await HubPage.summaryRowLink("calculated-summary-section")).click();
      await $(await FirstQuestionBlockPage.milk()).setValue(5);
      await $(await FirstQuestionBlockPage.eggs()).setValue(5);
      await $(await FirstQuestionBlockPage.bread()).setValue(5);
      await $(await FirstQuestionBlockPage.cheese()).setValue(5);
      await $(await FirstQuestionBlockPage.submit()).click();
      await $(await CurrencyTotalPlaybackPage.submit()).click();
      await $(await CalculatedSummarySectionSummaryPage.submit()).click();
    ...
    it("When a question in another section has a routing rule dependency on a calculated summary total, and the calculated summary total is greater than £100, then we should be routed to the second question block", async ()=> {
      await $(await HubPage.summaryRowLink("calculated-summary-section")).click();
      await $(await FirstQuestionBlockPage.milk()).setValue(30);
      await $(await FirstQuestionBlockPage.eggs()).setValue(30);
      await $(await FirstQuestionBlockPage.bread()).setValue(30);
      await $(await FirstQuestionBlockPage.cheese()).setValue(30);
      await $(await FirstQuestionBlockPage.submit()).click();
      await $(await CurrencyTotalPlaybackPage.submit()).click();
      await $(await CalculatedSummarySectionSummaryPage.submit()).click();

      await $(await HubPage.summaryRowLink("dependent-question-section")).click();
      await $(await VegetablesPage.yes()).click();
      await $(await VegetablesPage.submit()).click();
    ...
    it("When a question in another section has a routing rule dependency on a calculated summary total, and the calculated summary total is less than £100, then we should be routed to the section summary", async ()=> {
      await $(await HubPage.summaryRowLink("calculated-summary-section")).click();
      await $(await FirstQuestionBlockPage.milk()).setValue(20);
      await $(await FirstQuestionBlockPage.eggs()).setValue(20);
      await $(await FirstQuestionBlockPage.bread()).setValue(20);
      await $(await FirstQuestionBlockPage.cheese()).setValue(20);
      await $(await FirstQuestionBlockPage.submit()).click();
      await $(await CurrencyTotalPlaybackPage.submit()).click();
      await $(await CalculatedSummarySectionSummaryPage.submit()).click();
    ...
    it("When a question in another section has a dependency on a calculated summary total, and both sections are complete, and I go back and edit the calculated summary total, then the dependent section status should be in progress", async ()=> {
      ...
      await $(await VegetablesPage.yes()).click();
      await $(await VegetablesPage.submit()).click();
      await $(await DependentQuestionSectionSummaryPage.submit()).click();
      await expect(await $(await HubPage.summaryRowState("dependent-question-section")).getText()).to.equal("Completed");

      await $(await HubPage.summaryRowLink("calculated-summary-section")).click();
      await $(await CurrencyTotalPlaybackPage.milkAnswerEdit()).click();
      await $(await FirstQuestionBlockPage.milk()).setValue(100);
      await $(await FirstQuestionBlockPage.submit()).click();
      await $(await CurrencyTotalPlaybackPage.submit()).click();
      await $(await CalculatedSummarySectionSummaryPage.submit()).click();
      await expect(await $(await HubPage.summaryRowState("dependent-question-section")).getText()).to.equal("Partially completed");
    });
  });
});
