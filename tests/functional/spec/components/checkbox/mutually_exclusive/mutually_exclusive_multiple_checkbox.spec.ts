...
test.describe('Component: Mutually Exclusive Checkbox With Multiple Radio Override', () => {
  ...
  test.describe('Given the user has clicked multiple non-exclusive options', () => {
    ...
    test('When then user clicks the first mutually exclusive option, Then only the first mutually exclusive option should be checked.', async ({
      page
    }) => {
      const mandatoryCheckboxPage = new MandatoryCheckboxPage(page)
    ...
    test('When then user clicks the second mutually exclusive option, Then only the second mutually exclusive option should be checked.', async ({
      page
    }) => {
      const mandatoryCheckboxPage = new MandatoryCheckboxPage(page)
      const summaryPage = new SummaryPage(page)
