...
describe("Component: Mutually Exclusive Month Year Date With Single Checkbox Override", () => {
  ...
  describe("Given the user has entered a value for the non-exclusive month year date answer", () => {
    it("When then user clicks the mutually exclusive checkbox answer, Then only the mutually exclusive checkbox should be answered.", async ()=> {
      // Given
      await $(await MonthYearDatePage.monthYearDateMonth()).setValue("3");
      await $(await MonthYearDatePage.monthYearDateYear()).setValue("2018");
      await expect(await $(await MonthYearDatePage.monthYearDateMonth()).getValue()).to.contain("3");
      await expect(await $(await MonthYearDatePage.monthYearDateYear()).getValue()).to.contain("2018");

      // When
      await $(await MonthYearDatePage.monthYearDateExclusiveIPreferNotToSay()).click();

      // Then
      await expect(await $(await MonthYearDatePage.monthYearDateExclusiveIPreferNotToSay()).isSelected()).to.be.true;
      await expect(await $(await MonthYearDatePage.monthYearDateMonth()).getValue()).to.contain("");
      await expect(await $(await MonthYearDatePage.monthYearDateYear()).getValue()).to.contain("");

      await $(await MonthYearDatePage.submit()).click();

      await expect(await $(await SummaryPage.monthYearDateExclusiveAnswer()).getText()).to.have.string("I prefer not to say");
      await expect(await $(await SummaryPage.monthYearDateExclusiveAnswer()).getText()).to.not.have.string("March 2018");
    });
  });
  ...
  describe("Given the user has clicked the mutually exclusive checkbox answer", () => {
    it("When the user enters a value for the non-exclusive month year date answer and removes focus, Then only the non-exclusive month year date answer should be answered.", async ()=> {
      ...
      await expect(await $(await MonthYearDatePage.monthYearDateExclusiveIPreferNotToSay()).isSelected()).to.be.false;

      await $(await MonthYearDatePage.submit()).click();

      await expect(await $(await SummaryPage.monthYearDateAnswer()).getText()).to.have.string("March 2018");
      await expect(await $(await SummaryPage.monthYearDateAnswer()).getText()).to.not.have.string("I prefer not to say");
    });
  });
  ...
  describe("Given the user has not answered the question and the question is optional", () => {
    it("When the user clicks the Continue button, Then it should display `No answer provided`", async ()=> {
      ...
      // Then
      await expect(await $(await SummaryPage.monthYearDateAnswer()).getText()).to.contain("No answer provided");
    });
  });
});
