...
test.describe('Answer Action: Redirect To List Add Question (Radio)', () => {
  ...
  test.describe('Given the user is on a question with a "RedirectToListAddBlock" action enabled', () => {
    ...
    test.beforeAll('Launch survey', async ({ browser }) => {
      ...
      await openQuestionnaire('test_answer_action_redirect_to_list_add_block_radio.json')
    })
    ...
    test('When the user answers "No", Then, they should be taken to straight the list collector.', async () => {
    ...
    test('When the user answers "Yes" then they should be taken to the list collector add question.', async () => {
