...
test.describe('Feature: Routing - OR Operator', () => {
  test.describe('Equals', () => {
    test.describe('Given I start the or operator routing survey', () => {
      test.beforeEach(async ({ openQuestionnaire }) => {
        await openQuestionnaire('test_routing_or.json')
      })
      ...
      test('When I only enter the second answer correctly with 555 and 321, Then I should be routed to the correct page', async ({ page }) => {
        const correctAnswerPage = new CorrectAnswerPage(page)
        const firstNumberQuestionPage = new FirstNumberQuestionPage(page)
        const secondNumberQuestionPage = new SecondNumberQuestionPage(page)
      ...
      test('When I only enter the first answer correctly with 123 and 555, Then I should be routed to the correct page', async ({ page }) => {
        const correctAnswerPage = new CorrectAnswerPage(page)
        const firstNumberQuestionPage = new FirstNumberQuestionPage(page)
        const secondNumberQuestionPage = new SecondNumberQuestionPage(page)
        await firstNumberQuestionPage.answer1().fill('123')
