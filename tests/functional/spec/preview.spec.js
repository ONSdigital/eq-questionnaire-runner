...
describe("Introduction preview questions", () => {
  ...
  function testPreview(schema, page) {
    browser.openQuestionnaire(schema);
    $(page.previewQuestions()).click();
  ...
  it("Given I start a survey, When I view the preview page, Then all preview elements should be visible and any metadata piped answers are resolved", () => {
    testPreview(introductionSchemaHub, IntroductionPageHub);
    testPreview(introductionSchemaLinear, IntroductionPageLinear);
  });
  ...
  it("Given I start a survey, When I view the preview page of hub flow schema, Then the twisty button should read 'Show all' and answers should be invisible", () => {
    browser.openQuestionnaire(introductionSchemaHub);
    $(IntroductionPageHub.previewQuestions()).click();
    expect(browser.getUrl()).to.contain("questionnaire/preview");
    expect($(printButton).isClickable()).to.be.true;
    expect($(pdfButton).isClickable()).to.be.true;
