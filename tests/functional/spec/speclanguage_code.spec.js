...
describe("Language Code", () => {
  it("Given a launch language of Welsh, I should see Welsh text", async ()=> {
    ...
    await expect(await $(await NamePage.questionText()).getText()).to.contain("Rhowch enw");

    await $(await NamePage.firstName()).setValue("Catherine");
    await $(await NamePage.lastName()).setValue("Zeta-Jones");
    await $(await NamePage.submit()).click();

    await $(await DobPage.day()).setValue(25);
    await $(await DobPage.month()).setValue(9);
    await $(await DobPage.year()).setValue(1969);
    await $(await DobPage.submit()).click();

    await $(await NumberOfPeoplePage.numberOfPeople()).setValue(0);
    await $(await NumberOfPeoplePage.submit()).click();
    await $(await ConfirmNumberOfPeoplePage.yes()).click();
    await $(await ConfirmNumberOfPeoplePage.submit()).click();

    await expect(await $(await HubPage.heading()).getText()).to.contain("Teitl cyflwyno");
    await expect(await $(await HubPage.warning()).getText()).to.contain("Rhybudd cyflwyno");
    await expect(await $(await HubPage.guidance()).getText()).to.contain("Canllawiau cyflwyno");
    await expect(await $(await HubPage.submit()).getText()).to.contain("Botwm cyflwyno");
    await $(await HubPage.submit()).click();
  ...
  it("Given a launch language of English, I should see English text", async ()=> {
    ...
    await $(await HubPage.submit()).click();
    await expect(await $(await NamePage.questionText()).getText()).to.contain("Please enter a name");
    await $(await NamePage.firstName()).setValue("Catherine");
    await $(await NamePage.lastName()).setValue("Zeta-Jones");
    await $(await NamePage.submit()).click();
  ...
  it("Given a launch language of English, When I select Cymraeg, Then the language should be switched to Welsh", async ()=> {
    ...
    await $(await NamePage.switchLanguage("cy")).click();
    await expect(await $(await NamePage.questionText()).getText()).to.contain("Rhowch enw");
    await expect($("header").getText()).to.contain("Arolwg Iaith Prawf");
    await $(await NamePage.switchLanguage("en")).click();
    ...
    await expect(await $(await HubPage.guidance()).getText()).to.contain("Submission guidance");
    await expect(await $(await HubPage.submit()).getText()).to.contain("Submission button");
    await $(await HubPage.switchLanguage("cy")).click();
    await expect(await $(await HubPage.heading()).getText()).to.contain("Teitl cyflwyno");
    await expect(await $(await HubPage.warning()).getText()).to.contain("Rhybudd cyflwyno");
    await expect(await $(await HubPage.guidance()).getText()).to.contain("Canllawiau cyflwyno");
    await expect(await $(await HubPage.submit()).getText()).to.contain("Botwm cyflwyno");
  ...
  describe("Given a launch language of English and a question with plural forms, When I select switch languages, Then the plural forms are displayed correctly for the chosen language", () => {
    for (const dataSet of PLURAL_TEST_DATA_SETS) {
      ...
      it(`Test plural count: ${numberOfPeople}`, async ()=> {
        ...
        await $(await NumberOfPeoplePage.numberOfPeople()).setValue(numberOfPeople);
        await $(await NumberOfPeoplePage.submit()).click();

        await expect(await $(await ConfirmNumberOfPeoplePage.questionText()).getText()).to.contain(dataSet.question_title.en);
        await expect(await $(await ConfirmNumberOfPeoplePage.yesLabel()).getText()).to.contain(dataSet.answer.en);

        await $(await ConfirmNumberOfPeoplePage.switchLanguage("cy")).click();

        await expect(await $(await ConfirmNumberOfPeoplePage.questionText()).getText()).to.contain(dataSet.question_title.cy);
        await expect(await $(await ConfirmNumberOfPeoplePage.yesLabel()).getText()).to.contain(dataSet.answer.cy);
