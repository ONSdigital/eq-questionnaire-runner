...
describe("Thank You Business", () => {
  describe("Given I launch a business theme schema", () => {
    beforeEach(() => {
      browser.openQuestionnaire("test_theme_business.json");
    });

    it("When I navigate to the thank you page, Then I should see business theme content", () => {
      $(SubmitPage.submit()).click();
      $(HubPage.submit()).click();
      expect(browser.getUrl()).to.contain(ThankYouPage.pageName);
      expect($(ThankYouPage.title()).getHTML()).to.contain("Thank you for submitting the Test Business Survey");
