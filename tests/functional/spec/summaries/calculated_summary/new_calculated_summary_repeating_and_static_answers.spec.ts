...
test.describe('Calculated summary with repeating answers', () => {
  ...
  const dynamicAnswerChangeLink = (page: Page, answerIndex: number): ReturnType<Page['locator']> =>
    page.locator(summaryActions).nth(answerIndex).locator('a')
  ...
  test(
    ...
    async () => {
      ...
      await expect(calculatedSummaryVisitsPage.calculatedSummaryTitle()).toHaveText(
