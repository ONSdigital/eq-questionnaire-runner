...
describe("Component: Definition", () => {
  describe("Given I start a survey which contains question definition", () => {
    ...
    it('When I click the title link twice, then the description and "Hide this" button should not be visible', () => {
      ...
      // Then
      $(DefinitionPage.definitionContent(2)).waitForDisplayed({ timeout: 300, reverse: true });
      $(DefinitionPage.definitionButton(2)).waitForDisplayed({ timeout: 300, reverse: true });
    });
    ...
    it('When I click the title link then click "Hide this" button, then the description and button should not be visible', () => {
      ...
      // When
      $(DefinitionPage.definitionButton(2)).click();
    ...
    it('When I click the second definition\'s title link then the description and "Hide this" button for the second definition should be visible', () => {
      expect($(DefinitionPage.definitionContent(3)).isDisplayed()).to.be.false;
      expect($(DefinitionPage.definitionButton(3)).isDisplayed()).to.be.false;

      // When
      $(DefinitionPage.definitionTitle(3)).click();

      // Then
      $(DefinitionPage.definitionContent(3)).waitForDisplayed({ timeout: 300 });
      $(DefinitionPage.definitionButton(3)).waitForDisplayed({ timeout: 300 });
    });
  });
});
