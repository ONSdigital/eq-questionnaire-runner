...
class Router:
    ...
    def get_enabled_section_keys(
        ...
        for section_id in self.enabled_section_ids:
            ...
            elif repeating_blocks_list := self._schema.get_repeating_blocks_list_name_for_section(
                section_id
            ):
                for list_item_id in self._list_store[repeating_blocks_list]:
                    # Repeating blocks, which would not otherwise be tracked
                    yield section_id, list_item_id

                # Parent of repeating blocks
