...
class QuestionnaireStoreUpdaterBase:
    ...
    def is_dirty(self) -> bool:
        return bool(self._answer_store.is_dirty or self._list_store.is_dirty or self._progress_store.is_dirty)
    ...
    def remove_completed_relationship_locations_for_list_name(self, list_name: str) -> None:
        if target_relationship_collectors := self._get_relationship_collectors_by_list_name(list_name):
    ...
    def _update_relationship_question_completeness(self, list_name: str) -> None:
        relationship_collectors = self._get_relationship_collectors_by_list_name(list_name)
        ...
        for collector in relationship_collectors:
            relationship_answer_id = self._schema.get_first_answer_id_for_block(collector["id"])
            if relationship_answers := self._get_relationships_in_answer_store(relationship_answer_id):
                pairs = {(answer["list_item_id"], answer["to_list_item_id"]) for answer in relationship_answers}

                expected_pairs = set(combinations(list_items, 2))
                if expected_pairs == pairs:
                    section_id = self._schema.get_section_for_block_id(collector["id"])["id"]  # type: ignore
    ...
    def _get_relationships_in_answer_store(self, relationship_answer_id: str) -> list[RelationshipDict]:
    ...
    def capture_dependencies_for_list_change(self, list_name: str) -> None:
        ...
        section_ids = self._schema.get_when_rule_section_dependencies_for_list(list_name)
        section_ids.update(self._schema.list_collector_section_ids_by_list_name.get(list_name, set()))
        ...
        for section_key in self.started_section_keys(section_ids=section_ids):
            ...
            if section_key.list_item_id and not self._schema.get_repeat_for_section(section_key.section_id):
                continue
            self.dependent_sections.add(DependentSection(**section_key.to_dict()))

    def _get_relationship_answers_for_list_name(self, list_name: str) -> list[Answer] | None:
        associated_relationship_collectors = self._get_relationship_collectors_by_list_name(list_name)
        if not associated_relationship_collectors:
            return None

        relationship_answer_ids = [
            self._schema.get_first_answer_id_for_block(block["id"]) for block in associated_relationship_collectors
        ]

        return self._answer_store.get_answers_by_answer_id(relationship_answer_ids)

    def update_same_name_items(self, list_name: str, same_name_answer_ids: list[str] | None) -> None:
    ...
    def _remove_relationship_answers_for_list_item_id(self, list_item_id: str, answers: list) -> None:
        for answer in answers:
            answers_to_keep = [
                value for value in answer.value if list_item_id not in {value["to_list_item_id"], value["list_item_id"]}
            ]
    ...
    def update_section_status(self, *, is_complete: bool, section_key: SectionKey) -> bool:
        status = CompletionStatus.COMPLETED if is_complete else CompletionStatus.IN_PROGRESS
    ...
    def _update_answer(
        ...
        answer_value_to_store = (
            {key: value for key, value in answer_value.items() if value not in self.EMPTY_ANSWER_VALUES}
            if isinstance(answer_value, dict)
            else answer_value
        )

        if answer_value_to_store in self.EMPTY_ANSWER_VALUES:
            return self._answer_store.remove_answer(answer_id, list_item_id=list_item_id)
    ...
    def _capture_block_dependencies_for_list(self, list_name: str) -> None:
        ...
        dependencies: set[Dependent] = self._schema.list_dependencies.get(list_name, set())
    ...
    def _get_list_item_ids_for_list_dependency(self, dependency: Dependent) -> list[str] | list[None]:
        if dependency.for_list:
            return self._list_store[dependency.for_list].items
        return [None]

    def _capture_block_dependent(self, dependent: Dependent, list_item_ids: Sequence[str] | Sequence[None]) -> None:
        """
        ...
        for list_item_id in list_item_ids:
            if dependent.answer_id:
                self._answer_store.remove_answer(dependent.answer_id, list_item_id=list_item_id)
            self.dependent_block_id_by_section_key[SectionKey(dependent.section_id, list_item_id)].add(
                dependent.block_id
            )
        ...
        for section_id in answer_id_section_dependents.get(answer_id, {}):
            if repeating_list := self._schema.get_repeating_list_for_section(section_id):
                for list_item_id in self._list_store[repeating_list].items:
                    self.dependent_sections.add(DependentSection(section_id, list_item_id))
            else:
    ...
    def _capture_section_dependencies_progress_value_source_for_section(
        ...
        dependent_sections: Iterable = (
            self._schema.when_rules_section_dependencies_by_section_for_progress_value_source.get(section_id, set())
        )
        self._update_section_dependencies(dependent_sections)

    def _capture_section_dependencies_progress_value_source_for_block(self, *, section_id: str, block_id: str) -> None:
        """
        Captures a unique list of section ids that are dependents of the provided block, for progress value sources.
        """
        dependent_sections: Iterable = (
            self._schema.when_rules_block_dependencies_by_section_for_progress_value_source.get(section_id, {}).get(
                block_id, set()
            )
        )
        self._update_section_dependencies(dependent_sections)
    ...
    def update_progress_for_dependent_sections(self) -> None:
        ...
        for section in chronological_dependents:
            ...
            if (
                ...
                self._evaluate_dependents(dependent_section=section, evaluated_dependents=evaluated_dependents)
    ...
    def _evaluate_dependents(
        ...
        *,
        ...
        if is_path_complete is None:
            is_path_complete = self._router.is_path_complete(self._router.routing_path(dependent_section.section_key))

        if self.update_section_status(is_complete=is_path_complete, section_key=dependent_section.section_key):
            ...
            for dependent_section_id in dependents_of_dependent:
                if repeating_list := self._schema.get_repeating_list_for_section(dependent_section_id):
    ...
    def _capture_dependent_section(self, section_key: SectionKey) -> None:
        ...
        self.dependent_sections.add(DependentSection(**section_key.to_dict(), is_complete=False))

    def started_section_keys(self, section_ids: Iterable[str] | None = None) -> list[SectionKey]:
...
class QuestionnaireStoreUpdater(QuestionnaireStoreUpdaterBase):
    ...
    def __init__(
        ...
        super().__init__(schema=schema, questionnaire_store=questionnaire_store, router=router)
    ...
    def _capture_block_dependencies_for_answer(self, answer_id: str) -> None:
        ...
        dependencies: set[Dependent] = self._schema.answer_dependencies.get(answer_id, set())
        is_repeating_answer = self._schema.is_answer_in_repeating_section(answer_id)

        for dependent in dependencies:
            list_item_ids = self._get_list_item_ids_for_answer_dependency(dependent, is_repeating_answer)
            self._capture_block_dependent(dependent, list_item_ids)
    ...
    def update_answers(self, form_data: Mapping, list_item_id: str | None = None) -> None:
        list_item_id = list_item_id or self._current_location.list_item_id
        answers_by_answer_id = self._schema.get_answers_for_question_by_id(self._current_question)
    ...
    def capture_progress_section_dependencies(self) -> None:
        ...
        """
        self._capture_section_dependencies_progress_value_source_for_section(self._current_location.section_id)
