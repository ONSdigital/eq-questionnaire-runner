...
def get_env_or_fail(key):
    missing_key_error_message = f"Setting '{key}' Missing"
    value = os.getenv(key)
    if value is None:
        raise ValueError(missing_key_error_message)

    return value


def utcoffset_or_fail(date_value, key):
    datetime_offset_error_message = f"'{key}' datetime offset missing"
    if date_value.utcoffset() is None:
        raise ValueError(datetime_offset_error_message)
    return date_value
...
EQ_SUBMISSION_CONFIRMATION_QUEUE = os.getenv("EQ_SUBMISSION_CONFIRMATION_QUEUE", "eq-submission-confirmation")
...
EQ_GOOGLE_TAG_ID = os.getenv("EQ_GOOGLE_TAG_ID")
...
ADDRESS_LOOKUP_API_AUTH_ENABLED = parse_mode(os.getenv("ADDRESS_LOOKUP_API_AUTH_ENABLED", "False"))
