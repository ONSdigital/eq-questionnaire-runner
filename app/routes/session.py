...
def _set_questionnaire_supplementary_data(
    *,
    ...
    existing_sds_dataset_id = (
        questionnaire_store.stores.metadata.survey_metadata["sds_dataset_id"]
        if questionnaire_store.stores.metadata
        and questionnaire_store.stores.metadata.survey_metadata
        else None
    )
    ...
    if (
        ...
        _validate_supplementary_data_lists(
            supplementary_data=questionnaire_store.stores.supplementary_data_store.raw_data,
            schema=schema,
        )
        return
